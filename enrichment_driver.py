#!/usr/bin/env python
"""
Driver script to perform enrichment analysis.

Ruth McCole and Chamith Fonseka
May 12th 2017
"""
...
def get_args(strInput=None):
    ...
    """
    parser = argparse.ArgumentParser(description="This program allows you to run the enrichment.py script against "
    ...
    parser.add_argument('-g', '--genic', type=argparse.FileType('rU'),
                        help="A file containing genic UCEs")                    
...
def get_uces(args):
    ...
    aUCEFiles = []
    if args.all:
        if os.path.isfile('hg18_nonN_awk1based.txt'):
            allLen = len(list(args.all))
            aUCEFiles.append(('all', args.all.name, 'hg18_nonN_awk1based.txt', allLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.all.name)
            sys.exit(1)
    if args.intergenic:
        if os.path.isfile('hg18_intergenic_awk1based.bed'):
            interLen = len(list(args.intergenic))
            aUCEFiles.append(('intergenic', args.intergenic.name, 'hg18_intergenic_awk1based.bed', interLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.intergenic.name)
            sys.exit(1)
    if args.intronic:
        if os.path.isfile('hg18_introns_awk1based.bed'):
            inLen = len(list(args.intronic))
            aUCEFiles.append(('intronic', args.intronic.name, 'hg18_introns_awk1based.bed', inLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.intronic.name)
            sys.exit(1)
    if args.exonic:
        if os.path.isfile('hg18_exons_awk1based.txt'):
            exLen = len(list(args.exonic))
            aUCEFiles.append(('exonic', args.exonic.name, 'hg18_exons_awk1based.txt', exLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.exonic.name)
            sys.exit(1)
    if args.genic:
        if os.path.isfile('hg18_genic_awk1based.bed'):
            genLen = len(list(args.genic))
            aUCEFiles.append(('genic', args.genic.name, 'hg18_genic_awk1based.bed', genLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.genic.name)
            sys.exit(1)
    if len(aUCEFiles) == 0:
...
def run(inFile, aUCEs, cluster, debug, output):
    ...
    for tup in aUCEs:
        ...
        #Output individual results file
        strIndividualFilename = '{0}_depletion_results_for_{1}_UCEs.txt'.format(filename, tup[0])
        with open(strIndividualFilename, 'a+') as individualFh:
            individualFh.write("CNV Set\tUCE subset\telements\tn\tbp\tmean\ts.d.\tmin\tmax\tKSp-value\tKStestResult\tproportion\tp-value\tObs/Exp\tZtestResult\n")
            individualFh.write("{0}\t{1}\t{2}\t{3}\n".format(filename, tup[0], tup[3], "\t".join(map(str, aStats))))
